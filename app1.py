...
# Helper function to check valid numeric input
...
with tab1:    
    with st.form("my_form"):
        ...
        # Validation and Prediction for PREDICT SELLING PRICE
        flag = 0
        for i in [quantity_tons, thickness, width, customer]:             
            if not is_valid_numeric_input(i):
                flag = 1
                break
        
        if submit_button and flag == 1:
            st.write("You have entered an invalid value. Please enter a valid number (spaces are not allowed).")
             
        if submit_button and flag == 0:
            loaded_model, scaler_loaded, t_loaded, s_loaded = load_models()
...
with tab2: 
    with st.form("my_form1"):
        ...
        # Validation and Prediction for PREDICT STATUS
        cflag = 0
        for k in [cquantity_tons, cthickness, cwidth, ccustomer, cselling]:             
            if not is_valid_numeric_input(k):
                cflag = 1
                break
        ...
        if csubmit_button and cflag == 0:
            cloaded_model, cscaler_loaded, ct_loaded = load_classification_model()
